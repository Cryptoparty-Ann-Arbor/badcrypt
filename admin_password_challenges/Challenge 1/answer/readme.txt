...
random_data is a vector of pre-generated random data for use in hashing

key bit of code:

(a)    for (int i=0; i < pt.length(); i++)
    {
...
(b)	int index_i = dng(pt_sum, 0, 9); //determining index 1

    //pre-generated random data 10 entries, of 10 length
(c)   std::vector<std::string> random_data = {"3846870223", "3962687902", "7470852223", "7514179786", "2202953802", "7504486334", "1795029264", "7766294303", "6736111978", "9393656247"};
    

(d)   for (int i=0; i < 10; i++) // Craig doesn’t see the need for more than 10
    {
...
1. (a/b) Here we see that the initial index is determined by a sum of all the characters 
in the password. Also notice that the password is truncated to 10 characters.

2. (c) If the first index was index 0 ("3846870223") then dng(pt[i % pt.length()],0,9)] in an easier form to read dng(x,0,9)], x would have to be such that it returns a 0 or a 9 when run through the number generator, this would give us our expected first hash digit of 3.

3. (d) The index then changes by + 1 mod 10 (to keep wrapping around the random_data vector, therefore string 1 of random_data corresponding to index 1 would have to contain a 0 in order for index 0 to be a possible starting index. This is the second
character in the hash 3,0,0,6,3,9,4,6,0,8. Therefore the starting index can be checked by
following the increasing index and checking if the number exists in that index string. For example a starting index of 0 is quickly invalidated when we find that index 9's string does not contain the expected value of 8. (this is the last digit of the hash).
...
dng(pt[i % pt.length()],0,9) or dng(x, 0, 9) - where x is the span of ASCII decimal character table codes 33 through 136 - must correspond to indices that give the expected values.
...
And so on for indices 6, 7, 8, 9, 0, 1, 2, 3. Each one requiring a second index that will return the expected hashed result. This gives a series of numbers for the hash: 6-2-4-2-2-0-2-2-3-8. There may be other valid 2nd indices lists.
...
character length by running dng through 33 (the lowest decimal representation of a character used in the password creation) to 136*10 (10 times the largest decimal representation of a character, 10 because each potential character is considered). By considering only dng results that return the correct initial index (4), a list of possible pt_sum are found for the password.
...
Craig's hashing algorithm was very susceptible to analysis!
